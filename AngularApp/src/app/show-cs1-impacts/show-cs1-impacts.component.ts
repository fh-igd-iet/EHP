...
@Component({
  selector: 'app-show-cs1-impacts',
  templateUrl: './show-cs1-impacts.component.html',
  styleUrls: ['./show-cs1-impacts.component.css']
})
export class ShowCs1ImpactsComponent implements OnInit {
  ...
  selectedResultSubscription: Subscription = null;

  @ViewChild(ShowCs1ResultListComponent, { static: true }) resultList: ShowCs1ResultListComponent;  
  ...
  ngOnInit() {
    if (this.router.url.toLowerCase().includes('cs1')) {
      this.resultList.cs = 1
    }
    if (this.router.url.toLowerCase().includes('cs2')) {
      this.resultList.cs = 2
    }
    // loading selected lcaResults, indicators and methods from local storage 
    this.selectedResultSubscription = this.storageService.observeSelectedLcaResults().subscribe((results) => {
  ...
  updateRadarData() {
    ...
    if (this.selectedResults.length > 0 && this.selectedIndicators.length > 2 && this.currentTab == 0) {
      let m = []
      let p = []
      for (let pi of this.selectedResults)
        for (let mi of this.selectedMethods) {
          m.push(mi)
          p.push(pi)
        }
      this.radar_subscription = this.rest.get(`/process/result/${p.join(',')}/${m.join(',')}`).subscribe((d: LcaResult[]) => {
  ...
  resultsChanged(data: LcaResult[]) {
    ...
    this.selectedResults.push.apply(this.selectedResults, ids)
    this.updateColorPallette();
    this.cd.detectChanges()
    this.updateRadarData();
  ...
  updateColorPallette() {
    ...
    if (this.currentTab == 0)
      numColors = this.selectedResults.length
    else
    ...
    if (this.currentTab == 1) {
      ...
      this.resultList.setColorMode('grey');
